import { useEffect, useState } from 'react';
import { useNavigate, useParams } from 'react-router-dom';
import { ChevronLeft } from 'lucide-react';
import { studentApiClient } from '@/features/student/api/studentApiClient';

export default function SubmittedAssignmentPage() {
  const navigate = useNavigate();
  const { assignmentId } = useParams<{ assignmentId: string }>();

  const [assignment, setAssignment] = useState<any>(null);
  const [submission, setSubmission] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    if (!assignmentId) return;
    Promise.allSettled([
      studentApiClient.getAssignment(assignmentId),
      studentApiClient.getMySubmissions(),
    ]).then(([assignmentRes, submissionsRes]) => {
      if (assignmentRes.status === 'fulfilled') setAssignment(assignmentRes.value);
      if (submissionsRes.status === 'fulfilled') {
        const subs: any[] = submissionsRes.value;
        const match = subs.find(
          (s) => (s.assignmentId ?? s.assignment?._id ?? s.assignment?.id) === assignmentId
        );
        setSubmission(match ?? null);
      }
    }).finally(() => setIsLoading(false));
  }, [assignmentId]);

  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto space-y-6 animate-pulse">
        <div className="flex items-center gap-4 mb-8">
          <div className="w-10 h-10 bg-gray-100 rounded-xl" />
          <div className="space-y-2">
            <div className="h-7 w-56 bg-gray-100 rounded" />
            <div className="h-4 w-36 bg-gray-100 rounded" />
          </div>
        </div>
        <div className="h-48 bg-gray-100 rounded-2xl" />
      </div>
    );
  }

  if (!assignment) {
    return (
      <div className="flex items-center justify-center h-96">
        <p className="text-gray-500">Assignment not found</p>
      </div>
    );
  }

  const title = assignment.title ?? 'Assignment';
  const status = submission?.status ?? assignment.status ?? 'submitted';
  const submittedAt = submission?.submittedAt ?? submission?.createdAt ?? assignment.submittedAt;
  const score = submission?.score ?? assignment.score;
  const totalPoints = submission?.totalPoints ?? assignment.totalPoints;
  const feedback = submission?.feedback ?? submission?.remark ?? assignment.feedback;
  const suggestions = submission?.suggestions ?? assignment.suggestions;
  const questions: any[] = assignment.questions ?? [];
  const answers: any[] = submission?.answers ?? submission?.responses ?? [];

  const scorePercentage =
    score !== undefined && totalPoints ? Math.round((score / totalPoints) * 100) : 0;

  const getTimeSinceSubmission = () => {
    if (!submittedAt) return '';
    const diffMs = Date.now() - new Date(submittedAt).getTime();
    const diffHrs = Math.floor(diffMs / (1000 * 60 * 60));
    const diffMins = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
    return diffHrs > 0 ? `${diffHrs}hrs ${diffMins}mins ago` : `${diffMins}mins ago`;
  };

  return (
    <div className="max-w-4xl mx-auto space-y-6">
      {/* Header */}
      <div className="flex items-center gap-4 mb-8">
        <button
          onClick={() => navigate('/student/classroom')}
          className="w-10 h-10 flex items-center justify-center bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm group"
        >
          <ChevronLeft className="w-5 h-5 text-gray-500 group-hover:text-gray-700" />
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{title}</h1>
          <div className="flex items-center gap-3 text-sm mt-1">
            <span className={`px-2.5 py-0.5 rounded-full font-semibold text-xs ${
              status === 'graded' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
            }`}>
              {status === 'graded' ? 'Graded' : 'Submitted'}
            </span>
            {submittedAt && (
              <span className="text-gray-500">Submitted {getTimeSinceSubmission()}</span>
            )}
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Column: Questions & Answers */}
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            <h2 className="text-lg font-bold text-gray-900 mb-6">Your Response</h2>

            {questions.length > 0 ? (
              <div className="space-y-8">
                {questions.map((question: any, idx: number) => {
                  const answer = answers[idx];
                  const answerText =
                    typeof answer === 'string'
                      ? answer
                      : answer?.text ?? answer?.answer ?? answer?.content ?? '';
                  return (
                    <div key={question._id ?? question.id ?? idx} className="space-y-3">
                      <div className="flex gap-3">
                        <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-gray-100 rounded-md text-xs font-bold text-gray-600">
                          {idx + 1}
                        </span>
                        <p className="text-gray-900 font-medium">
                          {question.text ?? question.question ?? question.content ?? ''}
                        </p>
                      </div>
                      <div className="pl-9">
                        <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 text-gray-700 text-sm leading-relaxed">
                          {answerText || <span className="text-gray-400 italic">No answer submitted</span>}
                        </div>
                      </div>
                    </div>
                  );
                })}
              </div>
            ) : submission?.textContent ? (
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 text-gray-700 text-sm leading-relaxed">
                {submission.textContent}
              </div>
            ) : (
              <p className="text-gray-400 text-sm italic">No written response on record.</p>
            )}
          </div>
        </div>

        {/* Right Column: Score & Feedback */}
        <div className="lg:col-span-1 space-y-6">
          {status === 'graded' && score !== undefined && (
            <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
              <h2 className="text-lg font-bold text-gray-900 mb-6">Grade</h2>
              <div className="flex flex-col items-center justify-center py-4">
                <div className="relative w-32 h-32 mb-4">
                  <svg className="w-32 h-32 transform -rotate-90">
                    <circle cx="64" cy="64" r="56" stroke="#F3F4F6" strokeWidth="8" fill="none" />
                    <circle
                      cx="64"
                      cy="64"
                      r="56"
                      stroke={scorePercentage >= 70 ? '#10B981' : scorePercentage >= 50 ? '#F59E0B' : '#EF4444'}
                      strokeWidth="8"
                      fill="none"
                      strokeDasharray={`${(scorePercentage / 100) * 351.8} 351.8`}
                      strokeLinecap="round"
                    />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className={`text-3xl font-bold ${
                      scorePercentage >= 70 ? 'text-green-600' : scorePercentage >= 50 ? 'text-orange-500' : 'text-red-500'
                    }`}>
                      {scorePercentage}%
                    </span>
                    <span className="text-sm text-gray-400 font-medium">Score</span>
                  </div>
                </div>
                <div className="text-center">
                  <p className="text-gray-500 text-sm">Total Points</p>
                  <p className="text-xl font-bold text-gray-900">
                    {score}{' '}
                    <span className="text-gray-400 text-base font-normal">/ {totalPoints}</span>
                  </p>
                </div>
              </div>
            </div>
          )}

          {status === 'graded' && (
            <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm space-y-6">
              {feedback && (
                <div>
                  <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2">
                    <span>💬</span> Teacher's Remark
                  </h3>
                  <p className="text-sm text-gray-600 bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                    {feedback}
                  </p>
                </div>
              )}
              {suggestions && (
                <div>
                  <h3 className="font-bold text-gray-900 mb-2 flex items-center gap-2">
                    <span>💡</span> Suggestions
                  </h3>
                  <p className="text-sm text-gray-600 bg-orange-50/50 p-3 rounded-xl border border-orange-100">
                    {suggestions}
                  </p>
                </div>
              )}
            </div>
          )}

          <button
            onClick={() => navigate('/student/classroom')}
            className="w-full py-3 bg-gray-900 text-white font-semibold rounded-xl hover:bg-gray-800 transition-all shadow-sm"
          >
            Back to Classroom
          </button>
        </div>
      </div>
    </div>
  );
}
